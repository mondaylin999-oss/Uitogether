...
--
--  IDs are explicit so the file can be re-run safely
--  (ON CONFLICT DO NOTHING). Because the ids are supplied by hand, the
--  identity sequences do not advance on their own - the block at the very
--  bottom of this file fast-forwards them, without which the first real
...
-- ------------------------------------------------------------
INSERT INTO users (user_id, name, email, tnt, academic_year, password_hash, role) VALUES
...
(7, 'Ei Phyu',          'ei.phyu.dev@gmail.com',      'TNT-1006', 'Final Year', '$2b$12$MO6tVjN4V8zjUBIzhgWjbOAOJgV7GOJ4WxOVahSX2bGmyWjJ9GHoi', 'student') ON CONFLICT DO NOTHING;
...
(6, 7, 'EiEi',    'Semester 8', 'group_discussion', 'Machine Learning',            'Web Development, React',    'both',      'Final year project buddy wanted.',   '@eiphyu_web',    '+959444444444') ON CONFLICT DO NOTHING;
...
--   3  -> 7 : pending
-- ------------------------------------------------------------
INSERT INTO buddy_requests (request_id, sender_id, receiver_id, status) VALUES
(1, 2, 3, 'accepted'),
(2, 4, 2, 'pending'),
(3, 5, 6, 'rejected'),
(4, 3, 7, 'pending') ON CONFLICT DO NOTHING;
...
(3, 'Freshers Welcome Night',
    ...
    '2026-09-02', '17:00:00', 'UIT Open Ground', 'Student Union', NULL, 1) ON CONFLICT DO NOTHING;
...
(3, 6, 'lost',  'USB drive 32GB',          'SanDisk, has final year project files.',     'Library 3rd floor', '2026-08-20', NULL, 'Telegram @minkhant_net', 'resolved') ON CONFLICT DO NOTHING;
...
-- ------------------------------------------------------------
INSERT INTO polls (poll_id, question, description, status, ends_at, created_by) VALUES
(1, 'Which day works best for the weekly study meetup?', 'Pick one day. Results decide the schedule.', 'open',  '2026-12-31 23:59:59', 1),
(2, 'Best canteen snack at UIT?',                        'Just for fun.',                              'open',  NULL,                  1) ON CONFLICT DO NOTHING;
...
(5, 2, 'Mohinga',       1),
(6, 2, 'Samosa',        2),
(7, 2, 'Milk tea only', 3) ON CONFLICT DO NOTHING;

-- Poll 2 is closed at the end of this file, so on a RE-RUN it is already
-- closed and trg_votes_poll_open_bi would reject these rows outright - a
-- BEFORE INSERT trigger fires before ON CONFLICT can skip the duplicate.
-- (MySQL's INSERT IGNORE downgraded the trigger's SIGNAL to a warning, which
-- is why the original file did not need this.) Re-open it first.
UPDATE polls SET status = 'open' WHERE poll_id = 2;
...
(5, 2, 7, 2),
(6, 2, 5, 6),
(7, 2, 7, 7) ON CONFLICT DO NOTHING;
...
(8, 4, 'new_poll',               'New poll is open',       'Which day works best for the weekly study meetup?', 'poll',        1, 0) ON CONFLICT DO NOTHING;

-- ------------------------------------------------------------
-- Fast-forward every identity sequence past the hand-written ids above.
-- MySQL's AUTO_INCREMENT counter does this by itself; PostgreSQL's identity
-- sequence does not know about rows inserted with an explicit id, so without
-- this the next INSERT would try to reuse id 1 and hit a duplicate key.
-- ------------------------------------------------------------
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    SELECT table_name, column_name
      FROM (VALUES
        ('users',                'user_id'),
        ('study_buddy_profiles', 'profile_id'),
